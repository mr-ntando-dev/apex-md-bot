...
async function executeJob(sock, job) {
  ...
  switch (type) {
    ...
    // ── ai-chat (alias for 'ai' — dispatched by /api/handle-message for DMs) ─
    case 'ai-chat': {
      const { aiReply } = require('./ai');
      const { sender, body: msg, from } = payload;
      if (!msg) throw new Error('body required');
      const reply = await aiReply(sender, msg);
      if (reply && from) {
    ...
    // ── command — central router dispatched by /api/handle-message ──────────
    case 'command': {
      const cfg    = require('../config');
      const db     = require('./database');
      const { command, args = [], from, sender, isGroup, isOwner, key, message } = payload;
      if (!command || !from) throw new Error('command and from required');

      const prefix  = cfg.BOT_PREFIX || '.';
      const quoted  = key && message ? { key, message } : undefined;
      let replyText = null;

      // helper: send text and return
      const reply = async (text) => {
        await sock.sendMessage(from, { text }, quoted ? { quoted: { key, message } } : {});
      ...
      switch (command) {
        ...
        case 'ping': {
          const ms = Date.now();
          return reply(`🏓 Pong! Latency: *${Date.now() - ms}ms*`);
        }

        case 'runtime': {
          const s = Math.floor(process.uptime());
          return reply(`⏱️ *Uptime:* ${Math.floor(s/3600)}h ${Math.floor((s%3600)/60)}m ${s%60}s`);
        }

        case 'menu':
        case 'help': {
          const cats = [
            { e: '🤖', n: 'AI',         c: ['ai','analyze','imagine','voice','search','code','debate','roast','story','lyrics'] },
            { e: '🛡️', n: 'Admin',      c: ['warn','kick','promote','demote','tagall','mute','antilink','welcome','poll'] },
            { e: '📥', n: 'Downloader', c: ['tiktok','instagram','youtube','spotify','twitter','facebook','pinterest'] },
            { e: '🎮', n: 'Games',      c: ['slots','flip','tictactoe','daily','leaderboard','profile','quiz','work','rob'] },
            { e: '🔧', n: 'Utility',    c: ['weather','wikipedia','qr','translate','calc','ping','runtime','tts','shazam'] },
            { e: '💥', n: 'Fun',        c: ['joke','meme','8ball','quote','horoscope','fact','roll','burn','truth'] },
            { e: '🔒', n: 'Protection', c: ['anticall','antifake','antigm','antivv','antidemote'] },
            { e: '🎬', n: 'Media',      c: ['sticker','toaudio','toimg','logo','viewonce','audioeffects'] },
            { e: '👑', n: 'Owner',      c: ['ban','unban','mode','setprefix','restart','broadcast','plugins'] },
          ];
          const lines = cats.map(c =>
            `${c.e} *${c.n}*\n${c.c.map(x => `${prefix}${x}`).join('  ')}`
          );
          return reply([
            `⚡ *APEX-MD — Command Menu*`,
            `Prefix: *${prefix}*  |  v${cfg.BOT_VERSION}`,
            ``,
            lines.join('\n\n'),
          ].join('\n'));
        }
        ...
        case 'weather': {
          const r = await executeJob(sock, { type: 'utility/weather', payload: { city: args[0] || 'London' } });
          return reply(`🌤️ *${r.city}*\nTemp: ${r.temp}°C (feels ${r.feels}°C)\nCondition: ${r.condition}\nHumidity: ${r.humidity}%\nWind: ${r.wind} m/s`);
        }

        case 'translate': {
          if (!args.length) return reply(`❌ Usage: ${prefix}translate <text>`);
          const r = await executeJob(sock, { type: 'utility/translate', payload: { text: args.join(' '), target: 'en' } });
          return reply(`🌐 *Translation:*\n${r.translatedText}`);
        }

        case 'wikipedia':
        case 'wiki': {
          if (!args.length) return reply(`❌ Usage: ${prefix}wikipedia <query>`);
          const r = await executeJob(sock, { type: 'utility/wikipedia', payload: { query: args.join(' ') } });
          return reply(`📖 *${r.title}*\n\n${r.summary}\n\n🔗 ${r.url}`);
        }

        case 'qr': {
          if (!args.length) return reply(`❌ Usage: ${prefix}qr <text>`);
          const r = await executeJob(sock, { type: 'utility/qr', payload: { text: args.join(' ') } });
          if (r?.image) {
            await sock.sendMessage(from, { image: Buffer.from(r.image, 'base64'), caption: '✅ QR Code generated' });
            return { command, sent: true };
          }
          return reply('❌ QR generation failed.');
        }

        case 'calc': {
          if (!args.length) return reply(`❌ Usage: ${prefix}calc <expression>`);
          const r = await executeJob(sock, { type: 'utility/calc', payload: { expression: args.join(' ') } });
          return reply(`🧮 *${args.join(' ')}* = *${r.result}*`);
        }

        case 'ascii': {
          if (!args.length) return reply(`❌ Usage: ${prefix}ascii <text>`);
          const r = await executeJob(sock, { type: 'utility/ascii', payload: { text: args.join(' ') } });
          return reply(`\`\`\`\n${r.art}\n\`\`\``);
        }
        ...
        case 'base64': {
          ...
          if (!mode || !text) return reply(`❌ Usage: ${prefix}base64 encode|decode <text>`);
          const r = await executeJob(sock, { type: 'utility/base64', payload: { mode, text } });
          return reply(`🔡 *Result:*\n${r.result}`);
        }

        case 'tts': {
          if (!args.length) return reply(`❌ Usage: ${prefix}tts <text>`);
          const r = await executeJob(sock, { type: 'utility/tts', payload: { text: args.join(' '), lang: 'en' } });
          if (r?.audio) {
            await sock.sendMessage(from, { audio: Buffer.from(r.audio, 'base64'), mimetype: 'audio/mpeg' });
            return { command, sent: true };
          }
        ...
        // ── Fun ───────────────────────────────────────────────────────
        case 'joke': {
          const r = await executeJob(sock, { type: 'fun/joke', payload: {} });
          return reply(`😂 ${r.joke}`);
        }

        case 'meme': {
          const r = await executeJob(sock, { type: 'fun/meme', payload: {} });
          if (r?.url) {
            await sock.sendMessage(from, { image: { url: r.url }, caption: '😂' });
            return { command, sent: true };
          }
          return reply('❌ Could not fetch meme.');
        }

        case 'fact': {
          const r = await executeJob(sock, { type: 'fun/fact', payload: {} });
          return reply(`💡 ${r.fact}`);
        }

        case 'quote': {
          const r = await executeJob(sock, { type: 'fun/quote', payload: {} });
          return reply(`💬 _"${r.quote}"_\n— ${r.author}`);
        }

        case '8ball': {
          if (!args.length) return reply(`❌ Usage: ${prefix}8ball <question>`);
          const r = await executeJob(sock, { type: 'fun/8ball', payload: { question: args.join(' ') } });
          return reply(`🎱 ${r.answer}`);
        }

        case 'ship': {
          if (args.length < 2) return reply(`❌ Usage: ${prefix}ship <name1> <name2>`);
          const r = await executeJob(sock, { type: 'fun/ship', payload: { name1: args[0], name2: args[1] } });
          return reply(`💘 *${args[0]}* + *${args[1]}* = *${r.percent}%* compatible\n${r.bar}`);
        }

        case 'choose': {
          if (args.length < 2) return reply(`❌ Usage: ${prefix}choose <a> <b> ...`);
          const r = await executeJob(sock, { type: 'fun/choose', payload: { options: args } });
          return reply(`🤔 I choose: *${r.choice}*`);
        }

        case 'truth': {
          const r = await executeJob(sock, { type: 'fun/truth', payload: {} });
          return reply(`🫦 ${r.truth}`);
        }

        case 'horoscope': {
          if (!args[0]) return reply(`❌ Usage: ${prefix}horoscope <sign>`);
          const r = await executeJob(sock, { type: 'fun/horoscope', payload: { sign: args[0] } });
          return reply(`♈ *${r.sign}*\n${r.reading}`);
        }

        case 'roll': {
          const r = await executeJob(sock, { type: 'fun/roll', payload: { sides: Number(args[0]) || 6 } });
          return reply(`🎲 Rolled a ${r.sides}-sided die: *${r.result}*`);
        }

        // ── Games ─────────────────────────────────────────────────────
        case 'slots': {
          const r = await executeJob(sock, { type: 'games/slots', payload: { sender } });
          return reply(`🎰 ${r.display}\n${r.won ? `🎉 You won *${r.payout}* coins!` : '❌ Better luck next time.'}`);
        }

        case 'flip': {
          const r = await executeJob(sock, { type: 'games/flip', payload: {} });
          return reply(`🪙 *${r.result}*`);
        }

        case 'daily': {
          const r = await executeJob(sock, { type: 'games/daily', payload: { sender } });
          return reply(r.alreadyClaimed
            ? `⏳ Already claimed today. Next in: *${r.nextIn}*`
            : `💰 You claimed *${r.amount}* coins! Balance: *${r.balance}*`);
        }

        case 'work': {
          const r = await executeJob(sock, { type: 'games/work', payload: { sender } });
          return reply(r.cooldown
            ? `⏳ On cooldown. Try again in *${r.nextIn}*`
            : `💼 ${r.job} — You earned *${r.earned}* coins! Balance: *${r.balance}*`);
        }

        case 'profile': {
          const r = await executeJob(sock, { type: 'games/profile', payload: { sender } });
          return reply(`👤 *${r.name || sender.split('@')[0]}*\n💰 Balance: ${r.balance}\n🏆 Level: ${r.level}\n📊 Commands used: ${r.commandCount}`);
        }

        case 'leaderboard': {
          const r = await executeJob(sock, { type: 'games/leaderboard', payload: {} });
          const list = r.leaders.map((u, i) => `${i + 1}. ${u.name} — ${u.balance} coins`).join('\n');
          return reply(`🏆 *Leaderboard*\n\n${list}`);
        }

        case 'pay': {
          if (args.length < 2) return reply(`❌ Usage: ${prefix}pay <@number> <amount>`);
          const target = args[0].replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          const amount = Number(args[1]);
          if (!amount) return reply('❌ Invalid amount.');
          const r = await executeJob(sock, { type: 'games/pay', payload: { sender, target, amount } });
          return reply(r.success
            ? `✅ Paid *${amount}* coins to @${args[0].replace(/[^0-9]/g, '')}`
            : `❌ ${r.error || 'Payment failed.'}`);
        }

        case 'rob': {
          if (!args[0]) return reply(`❌ Usage: ${prefix}rob <@number>`);
          const target = args[0].replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          const r = await executeJob(sock, { type: 'games/rob', payload: { sender, target } });
          return reply(r.success
            ? `🦹 You robbed *${r.stolen}* coins from @${args[0].replace(/[^0-9]/g, '')}!`
            : `❌ ${r.error || 'Rob failed.'}`);
        }
        ...
        case 'ai':
        case 'chat': {
          if (!args.length) return reply(`❌ Usage: ${prefix}ai <message>`);
          const { aiReply } = require('./ai');
          const text = await aiReply(sender, args.join(' '));
          return reply(text || '🤖 No response from AI.');
        }

        case 'imagine': {
          if (!args.length) return reply(`❌ Usage: ${prefix}imagine <prompt>`);
          const r = await executeJob(sock, { type: 'ai', payload: { message: `/imagine ${args.join(' ')}`, userId: sender } });
          return reply(r?.reply || '🎨 Image generation failed.');
        }

        case 'analyze': {
          return reply('📸 Reply to an image with .analyze to describe it.');
        }
        ...
        case 'warn': {
          ...
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          if (!target || target === '@s.whatsapp.net') return reply(`❌ Usage: ${prefix}warn @number`);
          const r = await executeJob(sock, { type: 'admin/warn', payload: { groupId: from, sender: target, moderator: sender } });
          return reply(`⚠️ Warning ${r.count}/3 issued to @${args[0].replace(/[^0-9]/g, '')}`);
        }

        case 'kick': {
          if (!isGroup) return reply('👥 Groups only.');
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          if (!target || target === '@s.whatsapp.net') return reply(`❌ Usage: ${prefix}kick @number`);
          await executeJob(sock, { type: 'group/action', payload: { groupJid: from, participants: [target], action: 'remove' } });
          return reply(`🚫 Removed @${args[0].replace(/[^0-9]/g, '')} from the group.`);
        }

        case 'promote': {
          if (!isGroup) return reply('👥 Groups only.');
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          await executeJob(sock, { type: 'group/action', payload: { groupJid: from, participants: [target], action: 'promote' } });
          return reply(`⬆️ @${args[0].replace(/[^0-9]/g, '')} promoted to admin.`);
        }

        case 'demote': {
          if (!isGroup) return reply('👥 Groups only.');
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          await executeJob(sock, { type: 'group/action', payload: { groupJid: from, participants: [target], action: 'demote' } });
          return reply(`⬇️ @${args[0].replace(/[^0-9]/g, '')} demoted from admin.`);
        }

        case 'tagall': {
          if (!isGroup) return reply('👥 Groups only.');
          const r = await executeJob(sock, { type: 'admin/tagall', payload: { groupId: from, message: args.join(' ') || 'Attention!' } });
          return reply(r.text);
        }

        case 'mute': {
          if (!isGroup) return reply('👥 Groups only.');
          await executeJob(sock, { type: 'mute-group', payload: { groupJid: from, mute: true } });
          return reply('🔇 Group muted (admins only can message).');
        }

        case 'unmute': {
          if (!isGroup) return reply('👥 Groups only.');
          await executeJob(sock, { type: 'mute-group', payload: { groupJid: from, mute: false } });
          return reply('🔊 Group unmuted.');
        }

        case 'groupinfo': {
          if (!isGroup) return reply('👥 Groups only.');
          const r = await executeJob(sock, { type: 'admin/groupinfo', payload: { groupId: from } });
          return reply(`📋 *Group Info*\nName: ${r.name}\nMembers: ${r.memberCount}\nAdmins: ${r.adminCount}\nCreated: ${r.created}`);
        }

        case 'setdesc': {
          if (!isGroup) return reply('👥 Groups only.');
          if (!args.length) return reply(`❌ Usage: ${prefix}setdesc <description>`);
          await executeJob(sock, { type: 'admin/setdesc', payload: { groupJid: from, desc: args.join(' ') } });
        ...
        case 'setname': {
          ...
          await executeJob(sock, { type: 'admin/setname', payload: { groupJid: from, name: args.join(' ') } });
          return reply('✅ Group name updated.');
        }

        case 'warnings': {
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net' || sender;
          const r = await executeJob(sock, { type: 'admin/warnings', payload: { groupId: from, sender: target } });
          return reply(`⚠️ Warnings for @${target.split('@')[0]}: *${r.count}/3*`);
        }

        case 'resetwarn': {
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net' || sender;
          await executeJob(sock, { type: 'admin/resetwarn', payload: { groupId: from, sender: target } });
        ...
        case 'ban': {
          ...
          const target = args[0]?.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
          if (!target || target === '@s.whatsapp.net') return reply(`❌ Usage: ${prefix}ban @number`);
          const u = await db.getUser(target);
          u.banned = true;
          await db.setUser(target, u);
          return reply(`🚫 @${args[0].replace(/[^0-9]/g, '')} has been banned.`);
        }
        ...
        case 'unban': {
          ...
          u.banned = false;
          await db.setUser(target, u);
          return reply(`✅ @${args[0].replace(/[^0-9]/g, '')} has been unbanned.`);
        }

        case 'mode': {
          if (!isOwner) return reply('🔒 Owner only.');
          const m = args[0]?.toLowerCase();
          if (m === 'public')  { process.env.PUBLIC_MODE = 'true';  return reply('✅ Bot set to *Public* mode.'); }
          if (m === 'private') { process.env.PUBLIC_MODE = 'false'; return reply('✅ Bot set to *Private* mode.'); }
          return reply(`❌ Usage: ${prefix}mode public|private`);
        }

        case 'setprefix': {
          if (!isOwner) return reply('🔒 Owner only.');
          await executeJob(sock, { type: 'owner/setprefix', payload: { prefix: args[0] || '.' } });
        ...
        case 'restart': {
          ...
          await executeJob(sock, { type: 'owner/restart', payload: {} });
          return { command, sent: true };
        }

        case 'settheme': {
          if (!isOwner) return reply('🔒 Owner only.');
          await executeJob(sock, { type: 'bot/theme', payload: { theme: args[0] } });
          return reply(`✅ Theme set to *${args[0]}*`);
        }

        case 'plugins': {
          if (!isOwner) return reply('🔒 Owner only.');
          const r = await executeJob(sock, { type: 'plugins-list', payload: {} });
          if (!r.plugins.length) return reply('📦 No plugins installed.');
          const list = r.plugins.map(p => `• *${p.name}* — ${p.commands.join(', ')}`).join('\n');
        ...
        case 'broadcast': {
          ...
          const r = await executeJob(sock, { type: 'broadcast', payload: { message: args.join(' ') } }, 30000);
          return reply(`📢 Broadcast sent to *${r.sent}* groups.`);
        }
        ...
        case 'threads': {
          const url = args[0];
          if (!url) return reply(`❌ Usage: ${prefix}${command} <url>`);
          await sock.sendPresenceUpdate('composing', from);
          const r = await executeJob(sock, { type: 'download', payload: { url, type: 'video' } });
          if (r?.buffer) {
            await sock.sendMessage(from, {
              video: Buffer.from(r.buffer, 'base64'),
              mimetype: r.mimetype || 'video/mp4',
              fileName: r.filename || 'video.mp4',
              caption: `✅ Downloaded via APEX-MD`,
            });
            return { command, sent: true };
          }
          return reply('❌ Download failed. Check the URL.');
        }
        ...
        case 'soundcloud': {
          const url = args[0];
          if (!url) return reply(`❌ Usage: ${prefix}${command} <url>`);
          const r = await executeJob(sock, { type: 'download', payload: { url, type: 'audio' } });
          if (r?.buffer) {
            await sock.sendMessage(from, {
              audio: Buffer.from(r.buffer, 'base64'),
              mimetype: 'audio/mpeg',
        ...
        case 'youtube': {
          ...
          if (!query) return reply(`❌ Usage: ${prefix}ytmp4 <url or search>`);
          const r = await executeJob(sock, { type: 'download', payload: { url: query, type: 'video' } });
          if (r?.buffer) {
            await sock.sendMessage(from, {
              video: Buffer.from(r.buffer, 'base64'),
              mimetype: 'video/mp4',
              fileName: r.filename || 'video.mp4',
              caption: r.filename || 'YouTube video',
            });
            return { command, sent: true };
          }
          return reply('❌ Download failed.');
