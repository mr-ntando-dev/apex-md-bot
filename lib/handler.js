...
// ── Static command catalogue (loaded from API) ─────────────────
const commands = new Map();
const aliases  = new Map();

const COMMAND_CATALOGUE = [
  { category: 'admin', commands: ['antidelete','antilink','antispam','demote','filter','groupinfo','grouptools','hidetag','linkgroup','mute','poll','promote','resetwarn','revoke','setdesc','setname','tagall','unmute','warn','warnings','welcome'] },
  { category: 'ai', commands: ['analyze','character','chat','clearchat','code','debate','imagine','lyrics','roast','search','story','summarize','translate_ai','voice'] },
  { category: 'anime', commands: ['react'] },
  { category: 'business', commands: ['autorespond','broadcast'] },
  { category: 'downloader', commands: ['facebook','instagram','mediafire','pinterest','soundcloud','spotify','threads','tiktok','twitter','ytmp4'] },
  { category: 'fun', commands: ['8ball','burn','choose','fact','hangman','horoscope','joke','meme','quote','roll','ship','truth'] },
  { category: 'games', commands: ['daily','economy','flip','leaderboard','pay','profile','quiz','rob','slots','tictactoe','trivia','work'] },
  { category: 'media', commands: ['audioeffects','logo','play','sticker','toaudio','toimg','tts','viewonce'] },
  { category: 'owner', commands: ['ban','mode','plugins','restart','send','setprefix','settheme','unban'] },
  { category: 'protection', commands: ['anticall','antidemote','antifake','antigm','antivv'] },
  { category: 'utility', commands: ['ascii','base64','calc','currency','define','gitclone','help','ip','menu','morse','myip','password','ping','qr','remind','runtime','schedule','short','speed','translate','weather','whois'] },
];

function loadCommands() {
  // Commands are now handled via API job queue.
  // This static catalogue is for menu/help display only.
  let total = 0;
  for (const cat of COMMAND_CATALOGUE) {
    for (const name of cat.commands) {
      commands.set(name, {
        ...
        usage: `.${name}`,
        public: true,
        adminOnly: cat.category === 'admin',
        ownerOnly: cat.category === 'owner',
        groupOnly: cat.category === 'admin',
        execute: null, // executed via job queue
      });
      total++;
    }
  }
  logger.info(`[Handler] Registered ${total} commands (API job-queue mode)`);
}
...
async function handleMessage(sock, msg) {
  try {
    ...
    // ── Execute via job queue ──────────────────────────────
    await db.createJob('command', {
      command: cmdKey,
      args,
      from,
      sender,
      ...
      isOwner,
      body,
      quotedMsg: rawMsg.extendedTextMessage?.contextInfo?.quotedMessage || null,
    });
...
// ── Bad word check ────────────────────────────────────────────
const BAD_WORDS = ['spam', 'scam', 'fuck', 'shit', 'nigga', 'bastard'];
